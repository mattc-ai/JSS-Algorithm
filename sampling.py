...
def sampling(ins: dict,
    ...
    machines = ins['machines'].view(-1).to(device)
    encoder.eval()
    decoder.eval()
    ...
    # Schedule last job/operation and add it to the solution matrix
    last_jobs = mask.float().argmax(-1)
    last_ops = jsp.ops[_idx, last_jobs]
    last_m = machines[last_ops]
    last_idx = m_idx[_idx, last_m]
    sols[_idx, last_m, last_idx] = last_ops
    m_idx[_idx, last_m] += 1
    jsp(last_jobs, state)  # Update state with last operation
...
def greedy(ins: dict,
    ...
    # Schedule last job/operation and add it to the solution matrix
    last_jobs = mask.float().argmax(-1)
    last_ops = jsp.ops[0, last_jobs]  # Note: batch size is 1 for greedy
    last_m = machines[last_ops]
    last_idx = m_idx[last_m]
    sols[last_m, last_idx] = last_ops
    m_idx[last_m] += 1
    jsp(last_jobs, state)  # Update state with last operation
